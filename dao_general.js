        ...
        this.pool.getConnection((err, connection) => {
            if (err) {
                ...
                if (info.tipo === 'Protectora') {
                    sql = "SELECT id, email, password FROM protectora WHERE email=? AND password=? AND estado=?";
                }
                else if(info.tipo==='Adoptante'){
                    sql = "SELECT id, email, password FROM adoptante WHERE email=? AND password=? AND estado=?";
                }else if((info.tipo==='Administrador')){
                    sql = "SELECT id, email, password FROM administrador WHERE email=? AND password=?";
                }

                connection.query(sql, [info.user, info.password, 1], (err, result) => {
                    connection.release();
