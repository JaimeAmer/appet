...
class DAOPerro {
    ...
    updatePerro(datos, callback) {
        this.pool.getConnection((err, connection) => {
            ...
            if (datos.imagen == 1) { // fallecido=?, datos.texto.fallecido,
                connection.query("UPDATE perro SET nombre=?, edad=?, raza=?, color=?, peso=?, adoptado=?, descripcion=? WHERE id=?", 
                [datos.texto.nombre, datos.texto.edad, datos.texto.raza, datos.texto.color, datos.texto.peso, datos.texto.adoptado, 
                    datos.texto.descripcion, datos.texto.id], (err) => {
                    if (err) {
                        callback(err);
                        return;
                    }
            ...
            } else { //fallecido=?, datos.texto.fallecido,
                connection.query("UPDATE perro SET foto=?, nombre=?, edad=?, raza=?, color=?, peso=?, adoptado=?, descripcion=? WHERE id=?", 
                [datos.imagen, datos.texto.nombre, datos.texto.edad, datos.texto.raza, datos.texto.color, datos.texto.peso, 
                    datos.texto.adoptado, datos.texto.descripcion, datos.texto.id], (err) => {
                    if (err) {
                        callback(err);
                        return;
                    }
